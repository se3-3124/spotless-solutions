...
public class ServiceManager : IServiceManager
{
    ...
    public async Task<bool> UpdateAddonConfiguration(ServiceConfig update)
    {
        var service = _registry.GetActivatedAddonInstance(update.TargetingServiceId);
        if (service == null)
        {
            return false;
        }
        ...
        try
        {
            ...
            // Update the config
            service.UpdateConfiguration(update.Name, update.Description, serviceConfig.ServiceConfiguration);
        }
        catch (Exception ex)
        {
            _logger.LogInformation("Saving of updated service configuration failed: {ex}", ex);
